...
var SystemEXT = {
  _default: {
   ...
   ,config_folder: ""
   ,HTA_path: "platform/html/SystemAnimator_ie.hta"
...
var temp_config_xul_path = System.Gadget.path + toLocalPath('\\assets\temp\\_config_xul_path.tmp')
try {
  if (path) {
...
var temp_config_webkit_path = System.Gadget.path + toLocalPath('\\assets\temp\\_config_webkit_path.tmp')
try {
  if (path) {
...
var js =
  'var XUL_path;' + rn
+ 'var temp_config_xul_path = hta + "\\assets\temp\\_config_xul_path.tmp";' + rn
...
var js =
  'var WebKit_path;' + rn
+ 'var temp_config_webkit_path = hta + "\\assets\temp\\_config_webkit_path.tmp";' + rn
...
if (xul_mode) {
  XPCOM_object["Shell.Application"]._run(System.Gadget.path + '\\engine\\SA_xul_create_shortcut.js', encodeURIComponent(para[0])+' '+encodeURIComponent(para[1])+' '+encodeURIComponent(para[2])+' '+encodeURIComponent(para[3])+' '+encodeURIComponent(para[4]), true, true);
}
else if (webkit_mode) {
// supress occasional error when blocking is false
  WebKit_object["Shell.Application"]._run(System.Gadget.path + '\\engine\\SA_xul_create_shortcut.js', encodeURIComponent(para[0])+' '+encodeURIComponent(para[1])+' '+encodeURIComponent(para[2])+' '+encodeURIComponent(para[3])+' '+encodeURIComponent(para[4]), null, blocking);
}
else {
...
+ 'if (use_SA_browser_mode) { System.Gadget.Settings._settings = SystemEXT._default._settings; } else if (!System.Gadget.path) { System.Gadget.path = ' + gadget_path + '; };' + rn
  ...
  var c_js = (SA_demo) ? System.Gadget.path + toLocalPath('\\assets\temp\\_config_local\\' + cf.replace(/^.+[\/\\]/, "") + '.js') : ((use_config_local) ? System.Gadget.path + toLocalPath('\\assets\temp\\_config_local\\_SA_' + System._hash_sha256.hash(cf_full) + '.js') : config_folder + '_config_local.js')
