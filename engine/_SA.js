...
try {
  ...
  System.Gadget.settingsUI = "platform/html/settings.html"
...
,function (v) {
  ...
  var url = "platform/html/SystemAnimator_browse.html?title=" + encodeURIComponent("Choose an input file.")// + "&seed=" + Date.now()
  ...
  if ((use_SA_browser_mode || SA_animation_append_mode) && !EQP_gallery_append_mode) {
    ...
    if (folder_changed) {
      if (path_demo_by_url[path_folder])
        update_existing_config = ValidatePath(System.Gadget.path + '\\assets\temp\\_config_local\\' + path_folder.replace(/^.+[\/\\]/, "") + '.js')
      else if (webkit_mode && (path != path_folder))
        update_existing_config = ValidatePath(System.Gadget.path + '\\assets\temp\\_config_local\\_SA_' + System._hash_sha256.hash(path) + '.js')
      else
...
//System.Gadget.path + ((webkit_electron_mode) ? "\\electron_app" : "")
  ...
  args = [System.Gadget.path + toLocalPath("\\platform/html/SystemAnimator_ie.hta"), path_to_launch]
}
...
if (!self.oHTA) {
  if (webkit_electron_mode) {
    ...
    if (WallpaperEngine_mode) {
      ...
      fs.writeFileSync(System.Gadget.path + '\\assets\temp\\animation_path_default.txt', path_demo_by_url[path_to_launch]||path_to_launch)
    }
...
if (SystemEXT.enforce_WSH) {
  ...
  setTimeout(function () { Shell_OBJ.ShellExecute(System.Gadget.path + "\\engine\\SA_launcher.js", encodeURIComponent(args.join("|"))); setTimeout(function () { SA_top_window.close() }, 0); }, 0)
}
    }
    return
  }
...
if (!ie9)
  bg_w = bg_h = 1
      ...
      bg_src = "assets/images/_bg_dummy/1x1.png"
//DEBUG_show("(BG: OFF)", 2)
    }
    else {
      bg_src = "assets/images/bg.png"
...
document.write('<script language="JavaScript" src="plugins/effects/EQP.js"></scr'+'ipt>')
        }
        else {
          use_native_img = true
          gallery = [{frame:0, path:path, path_file:toFileProtocol(path)}]
...
// main
  ...
  if (MacFace_mode) {
    document.write(
  '<script language="JavaScript" src="vendor/PlistXMLParser.js"></scr'+'ipt>\n'
+ '<script language="JavaScript" src="engine/vistaFace.js"></scr'+'ipt>'
    )
    return
  }
  ...
  if (gallery_js)
    ...
    document.write('<script language="JavaScript" src="plugins/filters/animate.js"></scr'+'ipt>')
})();
