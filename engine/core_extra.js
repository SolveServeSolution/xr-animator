...
function SA_load_scripts() {
  ...
  if (use_SA_browser_mode) {
    ...
    if (is_SA_child_animation && params.f) {
      ...
      p = (params.cmd_line && params.cmd_line.split("|")) || ["/assets/temp/DEMO/miku01"]
...
//setTimeout('DEBUG_show("' + (WallpaperEngine_CEF_mode && System.Gadget.Settings.readString("animation_path_default.txt")) + '",0,1)',1000)
      if (!ani_path) {
        let animation_path_default = System.Gadget.path + toLocalPath('\\assets\temp\\animation_path_default.txt')
...
if (!WallpaperEngine_CEF_mode) {
  if (path_demo_by_url[SA_HTA_folder] || (SA_HTA_folder.indexOf(System.Gadget.path) != -1)) {
    c_js = System.Gadget.path + toLocalPath('\\assets\temp\\_config_local\\' + SA_HTA_folder.replace(/^.+[\/\\]/, "") + '.js');
    if (!FSO_OBJ.FileExists(c_js))
      c_js = "";
  }
  else if (webkit_mode && isFile) {
    c_js = System.Gadget.path + toLocalPath('\\assets\temp\\_config_local\\_SA_' + System._hash_sha256.hash(SA_HTA_folder_full) + '.js');
    if (!FSO_OBJ.FileExists(c_js))
      c_js = "";
  }
...
try {
  if (!SystemEXT.enforce_WSH)
    ...
    try {
      ...
      if (/^demo\d+/.test(f)) {
        f_config = null
//System.Shell.itemFromPath(System.Gadget.path + '\\assets\temp\\_config_local\\' + path_demo[f].replace(/^.+[\/\\]/, "") + '.js')
      }
      else {
  ...
  if (localhost_mode || (webkit_electron_mode && /AT_SystemAnimator_v0001\.gadget/.test(toLocalPath(self.location.href).replace(/[\/\\][^\/\\]+$/, "")))) {
    html +=
  '<script type="text/javascript" language="javascript" src="engine/dragdrop.js"></scr'+'ipt>\n'
+ '<script type="text/javascript" language="javascript" src="engine/img_cache.js"></scr'+'ipt>\n'
+ '<script type="text/javascript" language="javascript" src="engine/seq.js"></scr'+'ipt>\n'
+ '<script type="text/javascript" language="javascript" src="engine/shell_folder.js"></scr'+'ipt>\n'
...
console.log("_core.00.min.js")
    html +=
  '<script type="text/javascript" language="javascript" src="engine/_core.00.min.js"></scr'+'ipt>\n'
  }

  if (SA_project_JSON && SA_project_JSON.P2P_network && !is_SA_child_animation) {
console.log("Use PeerJS")
    html +=
  '<script type="text/javascript" language="javascript" src="vendor/peerjs.min.js"></scr'+'ipt>\n'
  }
...
console.log("Use Chatbox")
    html +=
  '<script type="text/javascript" language="javascript" src="plugins/social/chatbox.js"></scr'+'ipt>\n'
+ '<script>ChatboxAT.channel = SA_project_JSON.P2P_network.chatbox_channel||0;</scr'+'ipt>\n'
  }

  html +=
  '<script type="text/javascript" language="javascript" src="engine/_SA.js"></scr'+'ipt>\n'
+ '<script type="text/javascript" language="javascript" src="engine/_SA2.js"></scr'+'ipt>\n'
...
+ '<script type="text/javascript" language="javascript" src="engine/SA_gimage_emulation.js"></scr'+'ipt>\n'
...
//+ '   <img src="assets/images/icon_closing.png" class="QuickMenu_image" />\n'
...
//+ '   <img src="assets/images/icon_setting.png" class="QuickMenu_image" />\n'
+ '<span>⚙️</span>'
+ '  </div>\n'
...
//+ '   <img src="assets/images/icon_folder.png" class="QuickMenu_image" />\n'
+ '<span>📁</span>'
+ '  </div>\n'
+ '  <div class="QuickMenu_button" style="left:' + ((is_mobile)?'108px;width:36px;':'54px') + '" onclick="SA_OnDocument()" title="File input">\n'
//+ '   <img src="assets/images/icon_document.png" class="QuickMenu_image" />\n'
+ '<span>📰</span>'
+ '  </div>\n'
...
//+ '   <img src="assets/images/icon_gallery.png" class="QuickMenu_image" />\n'
+ '<span>🖼️</span>'
+ '  </div>\n'
...
System._browser.virtual_numpad(e, 'keyup')
    ...
    if (use_inline_dialog) {
      menu_html +=
  '<iframe id="Idialog" src="platform/html/z_blank2.html" frameborder="0" style="position:absolute; z-index:999; visibility:hidden;"></iframe>\n'
    }
  }
...
//    var config_js = ValidatePath(f_folder + '\\_config_local.js') || ValidatePath(System.Gadget.path + '\\assets\temp\\_config_local\\' + f_folder.replace(/^.+[\/\\]/, "") + '.js')
//    if (config_js)
