...
//wallpaper_src = System.Gadget.path + '/assets/images/zhen-yao-2bG6fFQDLLQ-unsplash.png';
...
//                cy = (Math.sign(cy) == Math.sign(cz)) ? Math.sign(cy) * Math.max(Math.abs(cy), Math.abs(cz)) : THREE.Math.clamp(cy + cz, -1,1);
    ...
    if (browser_native_mode) {
      ...
      wallpaper_src = toFileProtocol(System.Gadget.path + '/assets/images/2D-to-3D Wallpaper/wallpaper_default.jpg');
