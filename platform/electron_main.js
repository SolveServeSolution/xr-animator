...
//    global.is_transparent = false
  ...
  if (p) {
    ...
    const _animation_path_default_ = toLocalPath(SA_path + '/assets/temp/animation_path_default.txt');
...
if (path_demo_obj.path_demo_by_url[SA_HTA_folder]) {
  c_js = toLocalPath(SA_path + '\\assets\temp\\_config_local\\' + SA_HTA_folder.replace(/^.+[\/\\]/, "") + '.js')
  if (!fs.existsSync(c_js))
    c_js = ""
}
else if (isFile) {
  c_js = toLocalPath(SA_path + '\\assets\temp\\_config_local\\_SA_' + global.HASH_SHA256.hash(SA_HTA_folder_full) + '.js')
  if (!fs.existsSync(c_js))
    c_js = ""
}
...
webPreferences: webPreferences_default,
    ...
    mainWindow.loadURL((SA_path && toFileProtocol(SA_path + '\\' + 'platform/html/SystemAnimator_webkit.html')) || 'file://' + __dirname + '/index.html');
  }
  else {
...
//(SA_path && toFileProtocol(SA_path + '\\' + 'platform/html/SystemAnimator_webkit.html')) || 'file://' + __dirname + '/index.html');
//toFileProtocol("F:\\Programs Portable\\node-webkit\\AT_SystemAnimator_v8402.gadget\\platform/html/audio_BPM_detection_portable.html") + "?file=" + encodeURIComponent("C:\\Users\\user\\Desktop\\Youthful Days' Graffiti (Extended Mix).mp3"));
...
const DropArea_show = function (visible) {
  ...
  DropArea_window.loadURL(toFileProtocol(SA_path + "\\platform/html/drop_area.html"));
...
global.path_demo = (function () {
  ...
  return function () {
    if (!path_demo) {
      try {
        path_demo = JSON.parse(fs.readFileSync(toLocalPath(SA_path + '\\engine\\path_demo.json'), 'utf8'))
      }
      catch (err) {}
//electron.dialog.showErrorBox("",toLocalPath(SA_path + '\\engine\\path_demo.json'))
      if (!path_demo) {
